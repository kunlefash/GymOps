...
description: Activates the TEA Master Test Architect agent persona (Murat), runs the BMAD TEA test-design workflow, and publishes the resulting test design documents to planning-artifacts. Use when creating system-level or epic-level test design plans as part of Zone's Agentic SDLC process.
---

# Zone Test Design

Create comprehensive test design documents through the BMAD TEA workflow under the Master Test Architect agent persona, then publish them to planning-artifacts.
...
4. Also load `{project-root}/_bmad/bmm/config.yaml` and store `{project_name}` (needed for planning-artifacts title resolution)
...
`tests/e2e` (`modules/tests/e2e/`) is the **single canonical Playwright repository** for all E2E and API tests across the entire GymOps platform. `src/components-automation` is **deprecated** — if any loaded project artifact references it, treat it as `tests/e2e` in the generated output.

### Test Routing Rules (Apply to Every Coverage Plan and Execution Strategy)

| Test Level | Target Location | Notes |
|------------|----------------|-------|
| E2E | `modules/tests/e2e/tests/cardless/pwa/` | Browser-driven flows |
| API | `modules/tests/e2e/tests/cardless/api/` | HTTP/REST tests |
| Unit | Source module (e.g., `zone.gymops`, `zone.gymopspwa`) | Language-native: Jest (Next.js), Vitest (frontend), Hardhat (Solidity) |
| Integration | Source module | Same as unit |
...
### tests/e2e Conventions (Apply to Tooling, Code Examples, Infrastructure Sections)
...
If any input artifact (PRD, ADR, epic, story, prior test design) mentions `src/components-automation`:
1. Note the deprecation explicitly in the generated document's Assumptions section
2. Replace all tooling/infrastructure references with `tests/e2e` equivalents
3. Do NOT reproduce `src/components-automation` patterns (Index Facade POM, Allure reporting, PascalCase naming) in new test design output
...
- `planning-artifacts.title_prefix` (resolve `{project_name}` from BMAD config)
...
After a successful planning-artifacts publish, tag each local document with planning-artifacts space and page ID for easy retrieval and linking:

1. For each published document, ensure you have: `space_key` (from config), `mainPageId` (from 3.6), and the planning-artifacts base URL (from `getAccessibleAtlassianResources` or construct as `https://{site}.atlassian.net/wiki/spaces/{space_key}/pages/{mainPageId}`)
...
```bash
python3 .claude/skills/zone-dev/scripts/zone_dev.py commit-planning \
  --message "docs(test-design): tag test design documents with planning-artifacts publish metadata" --repo-root .
```
Stages all `_bmad-output/` changes (excludes `modules/`), commits, and pushes with retry. Skips automatically if no changes exist.
