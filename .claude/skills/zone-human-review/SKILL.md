...
description: Headless CI skill that reads a human reviewer's PR comments from Bitbucket, categorises them by severity (CRITICAL/HIGH/MEDIUM), appends them to the BMAD story file as review tasks, and transitions the Jira issue accordingly.
version: 1.0.0
triggers:
  keywords:
    - zone-human-review
...
Autonomous, CI-friendly skill that takes a Jira key and a Bitbucket PR URL, fetches human reviewer comments, categorises them by severity without user interaction, and appends them to the BMAD story file as tracked action items — mirroring the structure that `bmad-bmm-code-review` produces for AI reviews.

**Inputs**:
- `%jira_key%` — e.g. `BMAD-152`
- `%pr_url%` — e.g. `https://bitbucket.org/workspace/repo/pull-requests/42`
...
**If a genuine pre-existing blocker is found**: set `{blocker_summary}` = `"WORKSPACE_DIRTY: unexpected pre-existing dirty state — <list of dirty paths>"`, then run Phase 4.8 (Jira transition to Blocked) and Phase 4.7 (unconditional Jira comment) using `%jira_key%` as the issue key, then exit with status `"1"`. Do NOT proceed to Phase 1.
...
Capture JSON output as session variables: `{bmad_id}`, `{bmad_title}`, `{story_key}`, `{story_file_path}`, `{jira_key}`.
...
Set `{blocker_summary}` to `"<BLOCKER_TYPE>: <error message from script>"`, then run Phase 4.8 (Jira transition to Blocked) and Phase 4.7 (unconditional Jira comment) using `%jira_key%` as the issue key, then exit with status `"1"`. Do NOT proceed to Phase 2.

---

## Phase 2: Validate PR Inputs

No submodule checkout needed — code has already been reviewed by a human.
...
2. Parse `%pr_url%` to extract `{workspace}`, `{repo_slug}`, `{pr_id}`:
   - Expected pattern: `https://bitbucket.org/{workspace}/{repo_slug}/pull-requests/{pr_id}`
...
Note: The `commit-superrepo` command uses the commit message `{jira_key}: {bmad_title} - code review report`. This skill overrides that to use `human review report` — pass `--title "{bmad_title} - human review"` so the resulting commit message is `{jira_key}: {bmad_title} - human review - code review report`. Alternatively, accept the standard suffix from `commit-superrepo` — the important distinction is captured in the story file content.
...
python3 .claude/skills/zone-prepare-story/scripts/zone_prepare_story.py transition-jira \
  --jira-key {jira_key} --skill zone-human-review --outcome {outcome} --repo-root .
```
...
| No branch checkout | No submodule operations — story file is the only artifact written |
| Super-repo staging | Never `git add modules/*` — only `_bmad-output/` |
| Commit message | `{jira_key}: {bmad_title} - human review report` |
...
| Jira attachment | Attach story file on PASS only; best-effort, non-fatal |
| Phase 4.7 | **Always runs** — unconditional Jira comment with ✅/⚠️ result; never skipped |
