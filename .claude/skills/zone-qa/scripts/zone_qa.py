...
def jira_map_items(repo_root: Path) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    if not isinstance(items, list):
        die("jira-key-map.yaml projects.<active>.items must be a list")
...
def _prepare_one_submodule(
    ...
    """Prepare a single submodule: init, fetch, checkout/create target branch.
...
def cmd_prepare_branches(args: argparse.Namespace) -> int:
    ...
    target_branch = f"agent/story/{jira_key}-{story_key}"
    ...
    # in the canonical Playwright repo rather than inside the source submodule.
...
def cmd_prewarm(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        resolve_data: Dict[str, Any] = {
            ...
            "jira_key": jira_key, "qa_mode": qa_mode,
        }
        if epic_branch:
...
def cmd_commit_submodules(args: argparse.Namespace) -> int:
    ...
    commit_msg = f"{jira_key}: {title} - {suffix}"
    results = []
    committed_count = 0
    skipped_count = 0
...
def main() -> int:
    ...
    # prepare-branches
    p_branches = subparsers.add_parser("prepare-branches", help="Phase 2: Prepare submodule branches")
    ...
    p_branches.add_argument("--jira-key", required=True, help="Jira issue key")
    ...
    p_commit_super.add_argument("--suffix", default="test generation complete; submodule updates", help="Commit message suffix")
