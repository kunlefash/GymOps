---
name: zone-qa
description: Headless CI skill that resolves a Jira key to a BMAD story, generates acceptance/E2E/API/unit tests using TEA ATDD or BMM automation workflows, and commits/pushes test files per sub-repo and super-repo.
version: 1.0.0
triggers:
  keywords:
    - zone-qa
...
**NOTE**: When working on multiple submodules, you may dispatch bounded tasks to subagents per submodule/repo only inside Phase 3 after branch preparation has completed successfully. Never use subagents to overlap phases or to start submodule work before the Phase 2 gate has been satisfied.
...
- `prepare-branches` is a hard barrier for all submodule work.
- Do not run submodule tests, `dotnet test`, workflow commands, framework detection, subagent work, or implementation-oriented file inspection inside a prepared submodule until `prepare-branches` has returned successfully and `count > 0` has been confirmed.
- If `prepare-branches` is still running, wait. Do not speculate, preload Phase 3 work, or launch submodule validation in parallel.

## Modes

| Mode | When | Branch Pattern | Description |
|------|------|----------------|-------------|
| **ATDD** | Before `zone-dev` | `agent/story/{jira_key}-{story_key}` | Generates failing acceptance tests first; dev then implements code to make them pass |
| **automation** | After `zone-dev` | Reuses `agent/story/{jira_key}-{story_key}` | Generates tests to expand test coverage on existing implementation |
...
2. **Extract session variables** from the JSON in `PHASES 0-2 PRE-RESOLVED`:
   - `{bmad_id}`, `{bmad_title}`, `{story_key}`, `{story_branch}`,
     `{story_file_path}`, `{jira_key}`, `{qa_mode}`, `{epic_branch}`, `{initiative_branch}`, etc.
...
Run:
```
python3 .claude/skills/zone-qa/scripts/zone_qa.py sync-superrepo --repo-root .
```
...
python3 .claude/skills/zone-qa/scripts/zone_qa.py resolve \
  --jira-key %jira_key% [--qa-mode atdd|automation] --repo-root .
```

Capture JSON output as session variables: `{bmad_id}`, `{bmad_title}`, `{story_key}`, `{story_file_path}`, `{jira_key}`, `{qa_mode}`, `{epic_branch}`, `{parent_jira_key}`, `{initiative_branch}`.
...
Set `{blocker_summary}` to `"<BLOCKER_TYPE>: <error message from script>"`, then run Phase 5 (post blocked Jira comment) and Phase 5.5 (Jira transition to Blocked) using `%jira_key%` as the issue key and `{qa_mode}` = the supplied `--qa-mode` arg or `"automation"`, then exit with status `"1"`. Do NOT proceed to Phase 2.
...
python3 .claude/skills/zone-qa/scripts/zone_qa.py prepare-branches \
  ...
  --jira-key {jira_key} --story-key {story_key} \
...
Also capture `{domain_skills}` — the list of domain skill objects resolved from the prepared submodules.

Verify `count > 0` — at least one submodule was prepared. If count == 0:
- **Tier 3 ESCALATE-JIRA**: classify blocker based on mode:
  - ATDD mode: `EPIC_BRANCH_MISSING` — could not create test branches (epic branch not found)
  - automation mode: `BRANCH_NOT_FOUND` — no story branch found in any submodule
- Append to `{blocker_summary}`: e.g., `BRANCH_NOT_FOUND: story branch '{story_branch}' not found in any submodule (attempted: <list>)`.
...
If `count == 0`, skip directly to Phase 5. Do not run any submodule command.

**Branch behavior by mode**:
- **ATDD mode**: Creates `agent/story/{jira_key}-{story_key}` branches on test submodules (from epic branch or default). Uses the same branch namespace as zone-dev so the handoff is seamless.
- **automation mode**: Checks out existing `agent/story/{jira_key}-{story_key}` branches (already created by zone-dev). Submodules where the story branch does not exist are skipped.
...
Phase 3 begins only after Phase 2 and Phase 2.5 are complete. Do not begin workflow execution, framework detection, submodule inspection, or test commands until the Phase 2 prepare-branches barrier has completed successfully and `count > 0` is confirmed.
...
Load `.claude/skills/zone-qa/test-framework-map.yaml` to resolve each submodule's test framework, version, and conventions. Match the submodule name against the `pattern` field (pipe-delimited). Use the matched entry's `framework`, `domain_skill`, and `conventions` to configure test generation for that submodule.
...
Apply the following routing rules **before** executing the TEA workflow. This is a hard requirement: E2E and API tests MUST NOT be written into source submodules.

1. **Identify the E2E test repo**: From the Phase 2 `prepare-branches` output, find the submodule entry with `"role": "e2e_test_repo"`. This is always `zoneqa_automation`. If it is absent from the output (e.g., directory does not exist in this environment), log a warning and skip E2E/API test generation for those test types only — do not block unit/integration test generation.

2. **Test type routing rules**:

| Test Type | Target Submodule | Directory |
|-----------|-----------------|-----------|
| E2E (Playwright browser) | `zoneqa_automation` | `tests/cardless/pwa/` |
| API (Playwright request) | `zoneqa_automation` | `tests/cardless/api/` |
| Unit / Component | Source submodule (e.g., `zone.zonepaypwa`, `zone.zonepay`) | Per source module conventions |
| Integration | Source submodule | Per source module conventions |

3. **Convention injection for E2E/API tests** — when the TEA workflow generates E2E or API tests targeting `zoneqa_automation`, the following zone-qa-automation conventions MUST be applied. Treat these as absolute constraints, not suggestions:
...
4. **Convention injection for unit/integration tests** — keep `{project-root}` as the source submodule and follow the source module's domain skill (zone-dotnet for xUnit, zone-frontend for Vitest, etc.).
...
5. **Subagent context preamble** — when dispatching TEA subagents for E2E or API test generation, prepend the following to the subagent context before the subagent's own step file instructions:
   > "The target repository for all E2E and API tests is `modules/zoneqa_automation/`. Do NOT write test files into the source submodule. Use `tests/cardless/pwa/` for E2E tests and `tests/cardless/api/` for API tests. Follow the zone-qa-automation skill conventions: BasePage POM inheritance, `getHeadersForBank()` for API auth, multi-institution loops (BANKA/BANKB/OFI), `camelCase.spec.js` file names, and emoji logging."
...
Run:
```
python3 .claude/skills/zone-qa/scripts/zone_qa.py commit-submodules \
  --submodules '<JSON array of submodule paths from Phase 2>' \
...
Then commit super-repo:
```
python3 .claude/skills/zone-qa/scripts/zone_qa.py commit-superrepo \
  --jira-key {jira_key} --title "{bmad_title}" \
  --suffix "test generation complete; submodule updates" --repo-root .
```
...
## Phase 4.5: Attach Test Artifact to Jira (Best-Effort)
...
If the file exists, attach it to the Jira issue. Set `{artifact_attached}` = `true` on success, `false` otherwise.

Run (best-effort, non-fatal):
```
python3 .claude/skills/jira-agile/scripts/jira_agile.py attach-file {jira_key} \
...
## Phase 5: Jira Comment (Best-Effort)

Post a comment to the Jira issue summarizing test generation results. Branch on `{blocker_summary}`:
...
- Branch: `agent/story/{jira_key}-{story_key}`
...
**Best-effort**: If the comment call fails (e.g., no Jira connection in CI), log the failure and continue — non-fatal.

---

## Phase 5.5: Jira Transition
...
python3 .claude/skills/zone-prepare-story/scripts/zone_prepare_story.py transition-jira \
  --jira-key {jira_key} --skill zone-qa-{qa_mode} --outcome {outcome} --repo-root .
```
...
| Submodule branches first | Checkout/create test branches in all referenced submodules BEFORE the test generation workflow runs |
...
| Prepare-branches barrier | In automation mode, no submodule validation, build, or test command may run until `prepare-branches` has returned success and target branches are confirmed |
...
| Commit message | Always use the format: `{jira_key}: {bmad_title} - <suffix>` |
| Git PUSH | Ensure all local commits are pushed automatically. No further instructions required |
| HALT override | Three-tier protocol: Tier 3 (fundamental blockers) → ESCALATE-JIRA immediately, append to `{blocker_summary}`, skip to Phase 5 with `status: "1"`; Tier 2 (transient failures) → retry once with different approach, skip after 2 attempts; Tier 1 (self-healable) → retry once, skip item with log, continue — never terminate the workflow early |
...
| Test framework detection | Auto-detect test framework per submodule using domain skill mappings |
...
| Frontend deps in CI | When installing dependencies in frontend submodules (zone.clientdashboard, zone.zonepaypwa, zone.zonepay.qrrouter), use `npm ci` for npm modules and `yarn install --frozen-lockfile` for Yarn modules. Never run `npm install` or plain `yarn install` in CI — this regenerates lock files. |

### Sequencing Example

- Forbidden: run `prepare-branches` and `dotnet test` in parallel.
- Required: run `prepare-branches`, wait for completion, confirm `count > 0`, then start validation or workflow execution inside the prepared submodule branches.
...
| Submodule→skill map | `.claude/skills/zone-qa/submodule-skill-map.yaml` |
