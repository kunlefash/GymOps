---
name: zone-ux
description: Activates the UX Designer agent persona, runs the BMAD create-ux-design workflow, and publishes the resulting UX design specification to Confluence. Use when creating a new UX design or updating an existing one and pushing it to Confluence as part of Zone's Agentic SDLC process.
---

# Zone UX Design

Create a comprehensive UX design specification through the BMAD workflow under the UX Designer agent persona, then publish it to Confluence.
...
```bash
python3 .claude/skills/zone-dev/scripts/zone_dev.py commit-planning \
  --message "docs(ux): add Confluence link metadata" --repo-root .
```
Stages all `_bmad-output/` changes (excludes `modules/`), commits, and pushes with retry. Skips automatically if no changes exist.
