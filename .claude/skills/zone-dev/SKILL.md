---
name: zone-dev
description: Headless CI skill that resolves a Jira key to a BMAD story, executes the dev-story workflow in YOLO mode, and commits/pushes changes per sub-repo and super-repo.
version: 2.0.0
triggers:
  keywords:
    - zone-dev
...
Autonomous, CI-friendly skill that takes a Jira key, resolves it to a BMAD story, runs the full dev-story workflow without user interaction, and commits results to the appropriate branches.
...
**NOTE**: When working on multiple submodules, you should dispatch the tasks to subagents per submodule/repo. This will greatly improve speed.
...
2. **Extract session variables** from the JSON in `PHASES 0-2 PRE-RESOLVED`:
   - `{bmad_id}`, `{bmad_title}`, `{story_key}`, `{story_branch}`,
     `{story_file_path}`, `{jira_key}`, `{epic_branch}`, `{initiative_branch}`, etc.
...
Capture JSON output as session variables: `{bmad_id}`, `{bmad_title}`, `{story_key}`, `{story_branch}`, `{story_file_path}`, `{jira_key}`, `{epic_branch}`, `{parent_jira_key}`, `{initiative_branch}`.
...
Set `{blocker_summary}` to `"<BLOCKER_TYPE>: <error message from script>"`, then run Phase 4.5 (post blocked Jira comment) and Phase 5.5 (Jira transition to Blocked) using `%jira_key%` as the issue key, then exit with status `"1"`. Do NOT proceed to Phase 2.
...
Capture JSON output. Verify `count > 0` — at least one submodule was prepared.
Also capture `{domain_skills}` — the list of domain skill objects resolved from the prepared submodules.
All subsequent dev work in Phase 3 will now land directly on the story branch in each submodule.
...
1. **Load the Dev Agent persona** — apply `/bmad-agent-bmm-dev`:
   ...
   e. **Apply domain skill standards** — all code written in Phase 3 MUST conform to the coding standards loaded in Phase 2.5. When working in a submodule, follow the patterns from its mapped domain skill (e.g., Clean Architecture for .NET modules, React 18.3 + MUI patterns for frontend modules). If a domain skill references a supplementary document needed for the current task, load it on-demand.
...
5. **YOLO mode rules** — these override ALL workflow prompts, gates, AND HALT conditions:
   ...
   - **Three-tier HALT protocol** — HALT triggers are classified by tier and handled differently. NEVER terminate the workflow early.
     ...
     | **Tier 2 — Transient** | 3 consecutive task implementation failures, framework detection failure | FIX-THEN-SKIP (2 additional attempts), then ESCALATE-JIRA if still failing |
     | **Tier 3 — Fundamental Blockers** | Ambiguous requirements, missing config/secrets, new out-of-scope dependencies, Step 9 DoD gate failures, story file inaccessible | ESCALATE-JIRA immediately (1 fix attempt is sufficient to confirm irresolvable) |
     ...
     **ESCALATE-JIRA protocol** (Tier 2 confirmed irresolvable; Tier 3 immediately after 1 attempt):
     ...
     (4) **Guardrail**: Events are collected in `{blocker_summary}` — the actual Jira comment is posted in Phase 4.5 (not inline). A Jira API failure in Phase 4.5 MUST NOT stop the workflow.
   ...
   - **Cross-submodule persistence**: When tasks span multiple submodules (e.g., writing tests in zone.clientdashboard, integrating with zone.framework), switch context to each submodule as needed. All submodules were prepared in Phase 2.
...
8. **Serial task loop** — Process items strictly in sequence:
   ...
   d. If the item is completed, check it `[x]`. If it truly cannot be completed (e.g., missing infrastructure not available in any submodule), leave it `[ ]` and log the reason in Dev Agent Record.
   e. Move to the next item. **Do not stop, summarize, or wrap up until the queue is empty.**
   f. Commit and push the changes to the sub--repo as soon as every item is completed in the sub-repo following the submodule and subrepo rules.
...
Run:
```
python3 .claude/skills/zone-dev/scripts/zone_dev.py commit-submodules \
  --submodules '<JSON array of submodule paths from Phase 2>' \
  --story-branch {story_branch} --jira-key {jira_key} \
...
## Phase 4.5: Post Jira Dev Completion Comment

Before running the comment command, prepare two values:

**Extract `{committed_submodule_list}`**: From Phase 4's `commit-submodules` JSON output, collect the `name` field of every submodule entry where `action` is `"committed"`, joined as a comma-separated string (e.g., `zone.framework, zone.zonepay`). If no submodules were committed or Phase 4 output is unavailable, use `"none"`.
...
- Submodules changed: {committed_submodule_list}
...
- `{blocker_summary}` = empty string if no blockers, else:
  `## Blockers` followed by one markdown bullet per ESCALATE-JIRA event.
  Format each blocker as: `- [{task_id}] **{type}**: {reason}. Action needed: {guidance}.`

The same enrichment (branch, submodule list, dev agent record body) applies to the blocked/partial comment variant. If the story file is inaccessible at that point, omit `{dev_agent_record_body}`.
...
Run:
```
python3 .claude/skills/zone-dev/scripts/zone_dev.py commit-superrepo \
...
python3 .claude/skills/zone-prepare-story/scripts/zone_prepare_story.py transition-jira \
  --jira-key {jira_key} --skill zone-dev --outcome {outcome} --repo-root .
```
...
| Submodule branches first | Checkout story branches in all referenced submodules BEFORE the dev-story workflow runs, so all code changes land on the correct branch |
| Submodule init | Only initialize submodules that the story file references in Tasks/Subtasks |
| Initiative branch first | If the epic belongs to an initiative (per sprint-status.yaml `initiatives` section), ensure initiative branch exists on remote (from submodule default branch) before creating epic branches; push immediately after creation |
| Epic branch first | Ensure epic branch exists on remote (from initiative branch if tagged, otherwise submodule default branch) before creating new story branches; push immediately after creation; skip if story branch already exists |
...
| Submodule→skill map | `.claude/skills/zone-dev/submodule-skill-map.yaml` |
