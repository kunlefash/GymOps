---
name: zone-prd
description: Activates the PM agent persona, runs the BMAD create-prd workflow, and publishes the resulting PRD to planning-artifacts. Use when creating a new PRD or updating an existing one and pushing it to planning-artifacts as part of Zone's Agentic SDLC process.
---

# Zone PRD

Create a comprehensive PRD through the BMAD workflow under the PM agent persona, then publish it to planning-artifacts.
...
```bash
python3 .claude/skills/zone-dev/scripts/zone_dev.py commit-planning \
  --message "docs(prd): add planning-artifacts link metadata" --repo-root .
```
Stages all `_bmad-output/` changes (excludes `modules/`), commits, and pushes with retry. Skips automatically if no changes exist.
